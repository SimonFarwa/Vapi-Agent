...
**Offen:** (1) Echter Testanruf durch Simon an 08272 1819444 — finale Bestätigung, dass Vapi die Tool-Args im erwarteten Format schickt (E2E-Sim schon grün). (2) UTC→Berlin-Zeitkonvertierung für Slot-Ansage (Handler gibt UTC roh zurück). (3) Aussprache „Stengelmair" (TTS, Phase 2, empirisch). (4) Vapi-Key rotieren (im Chat exponiert). Optional später: LLM auf `gemini-2.5-flash`/Mistral testen. Ein einzelner Assistant reicht (kein Squad).
...
- **Bekannte Folge-Baustelle:** cal.com liefert Slots in **UTC**; Handler gibt sie roh zurück. Irene könnte Zeiten falsch ansagen (UTC statt Berlin). Fix (Prompt-Hinweis „UTC→Europe/Berlin umrechnen" oder Konvertierung im n8n-Node) noch offen. Ebenso Aussprache „Stengelmair" (TTS, Phase 2).
